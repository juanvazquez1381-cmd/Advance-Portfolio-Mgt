...
def get_ind_file(filetype, weighting="vw", n_inds=30):
    ...
    ind = pd.read_csv(f"ind{n_inds}_m_{name}.csv", header=0, index_col=0, na_values=-99.99)/divisor
...
def compound(r):
    """"
    return the result of compounding the set of returns in r
    """
...
def plot_ef2(n_points, er, cov):
    ...
    """
    if er.shape[0] != 2:
        raise ValueError("plot_eftw can only plot 2-asset frontiers")
...
def optimal_weights(n_points, er, cov):
    """
    generate list of weights to run the optimizer on to minimize the vol
    """
...
def run_cppi(risky_r, safe_r = None, m = 3, start = 1000, floor = 0.8, riskfree_rate = 0.03, drawdown = None):
    ...
    if safe_r is None:
        ...
        safe_r.values[:] = (1 + riskfree_rate)**(1/12) - 1
...
def summary_stats(r, riskfree_rate = 0.03):
    """
    Return a DataFrame that contains aggregated summary stats for the returns of columns r
    """
...
def gbm(n_years = 10, n_scenarios=1000, mu=0.07, sigma=0.15, steps_per_year=12, s_0=100.0, prices = True):
    ...
    rets = np.random.normal(loc=loc, scale=scale, size=(n_steps, n_scenarios))
    rets[0] = 0
    ret_val = s_0 * pd.DataFrame(np.exp(rets.cumsum())) if prices else rets
    return ret_val
...
def discount(t, r):
    ...
    """
    # Changed from pd.DataFrame to pd.Series
    discounts = pd.Series(data=[(r + 1)**-i for i in t], index=t)
    return discounts
...
def pv(flows, r):
    ...
    # Simple Series multiplication followed by sum()
...
def bond_cash_flows(maturity, principal = 100, coupon_rate = 0.03, coupons_per_year = 12):
    ...
    cash_flows.iloc[-1] += principal # add the principal of the last payment
    return cash_flows
...
def bond_total_return(monthly_prices, principal, coupon_rate, coupons_per_year):
    ...
    """
    # Initialize as float (0.0) to handle decimal coupon payments
...
def macaulay_duration(flows, discount_rate):
    ...
    total_price = discounted_flows.sum()
    weights = discounted_flows / total_price
    # Since weights is now a Series, we pass it directly
...
def match_durations(cf_t, cf_s, cf_l, discount_rate):
    ...
    """
    d_t = macaulay_duration(cf_t, discount_rate) # targeted duration
    d_s = macaulay_duration(cf_s, discount_rate) # short duration
    d_l = macaulay_duration(cf_l, discount_rate) # longer term duration
...
def target_risk_contributions(target_risk, cov):
    ...
    weights = minimize(msd_risk, init_guess, args=(target_risk, cov), 
                       method='SLSQP', constraints=(weights_sum_to_1,), bounds=bounds)
    return weights.x
